...
/// Already initialized, synchronous arithmetic-only prefix owner.
/// Implementations must validate their current row/round state without mutation.
/// Caller owns any earlier frontend transcript mutation; errors abort that attempt.
/// This interface deliberately has no begin method: ownership cannot be imported twice.
pub trait StartedFoldBackend {
    fn admit_initialized(&self, rows: usize, rounds: usize) -> Result<(), String>;
    fn advance(&mut self, rho: F128) -> Result<SumcheckMessage, String>;
    /// Consume/export exactly the already-folded planes. No extra fold or replay.
    fn finish(&mut self) -> Result<(Vec<F128>, Vec<F128>), String>;
}

/// Continue an already produced basis/round0 without allocating a dummy full basis.
/// Scalar transcript state matches prove_with_backend after begin. The trusted
/// backend owns the initial planes and must bind round0 to those exact inputs.
/// Admission is before this entry changes the challenger; device errors are fatal.
#[allow(clippy::too_many_arguments)]
pub fn prove_with_started_backend<Ch: Challenger>(
    config: &ProverConfig,
    rows: usize,
    target: F128,
    l0_codeword: &[F128],
    l0_tree: &[Hash],
    start_msg: SumcheckMessage,
    challenger: &mut Ch,
    backend: &mut dyn StartedFoldBackend,
) -> Result<LigeritoProof, String> {
    if rows < 4
        || !rows.is_power_of_two()
        || config.initial_k == 0
    ...
    backend.admit_initialized(rows, config.initial_k)?;
    let log_n = rows.trailing_zeros() as usize;
    let r = config.recursive_steps;
    let initial_k = config.initial_k;
    ...
    let _t = std::time::Instant::now();
    // Metadata only: native backend already owns both initialized planes.
    // This is exactly new_with_first_msg state after the old entry takes planes.
    let mut sc_prover = SumcheckProver {
        f: Vec::new(), combined_basis: Vec::new(), t_r: target,
        transcript: vec![start_msg], pending_glue: None,
    };
    challenger.observe_f128(start_msg.u_0);
    challenger.observe_f128(start_msg.u_2);
